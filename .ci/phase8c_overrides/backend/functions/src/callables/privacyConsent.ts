...
function activeSessionId(data: Record<string, unknown> | undefined): string | null {
  const raw = data?.activeShareSessionId;
  return typeof raw === "string" && raw.trim().length > 0 ? raw.trim() : null;
}

function safeNextEpoch(data: Record<string, unknown> | undefined): number {
  const next = privacyEpoch(data) + 1;
...
async function invalidateOwnerPrivacy(ownerUserId: string): Promise<void> {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (sessionId !== null) {
      transaction.set(
        ownerRef.collection("shareSessions").doc(sessionId),
        {
          active: false,
    ...
    transaction.set(
      ownerRef,
      {
        ...
        privacyEpoch: safeNextEpoch(data),
...
export const setPrivacyMode = onCall(callableOptions, async (request) => {
  ...
  const liveLocationRef = db.doc(`liveLocations/${uid}`);
  const nowMs = Date.now();
  await db.runTransaction(async (transaction) => {
    const snapshot = await transaction.get(ownerRef);
    ...
    if (invalidatesSession && sessionId !== null) {
      transaction.set(
        ownerRef.collection("shareSessions").doc(sessionId),
        {
          active: false,
    ...
    transaction.set(
      ownerRef,
      {
        ...
        liveSharingEnabled: false,
        activeShareSessionId: null,
        privacyEpoch: invalidatesSession
          ? safeNextEpoch(current)
          : privacyEpoch(current),
        updatedAt: FieldValue.serverTimestamp(),
      },
      { merge: true },
    );
    if (invalidatesSession) transaction.delete(liveLocationRef);
  });
  return { ok: true };
});
